...
# to run script with a specific port, and being verbose:
# Rscript CCISS.R -p <port> -v
options(warn=-1)
...
require(tcltk, quietly = TRUE, warn.conflicts = FALSE)
...
require(compiler, quietly = TRUE, warn.conflicts = FALSE)
enableJIT(3)
#===============================================================================
...
# if (length(args)<3) {
#     stop("Three arguments must be supplied e.g. <workdir> <average> <region>", call.=FALSE)
# }
...
parser$add_argument("-v", "--verbose", action="store_true", default=FALSE,
    help="Be verbose with output [default]")
parser$add_argument("-p", "--port", type="integer", default=7433, #kinda looks like TREE?
    help="Port to listen on for zeroMQ JSON information")

args <- parser$parse_args()

port = args$port
verbose = args$verbose # "Yes" / "No"

## added underscores into the region names in the database for a clean parameter input
## replace the underscore with a space in order to match the CSV: ./StockStands_v11.csv
region = 'Pr_George'

###Function to find the proportion edatopic overlap###
edaOverlapU <- function(dat1, dat2){
    return(length(dat1[dat1 %in% dat2])/length(dat2))
}

edaOverlap <- cmpfun(edaOverlapU)

######Functions used in suitability rules####

bifurcTrendU <- function(x){ ##percent improve, stable, and same - bifurcated model in summary data
    if(x[1] >= 25 & x[3] >= 25){
        return("TRUE")
    }else{
        return("FALSE")
...
bifurcTrend <- cmpfun(bifurcTrendU)

newSuitU <- function(x){ ##Calculate new suitability accounting for current suitability
...
newSuit <- cmpfun(newSuitU)

newSuitnoCurrentU <- function(x){ ##New suitability without current suitability
  return(1*x[1]+2*x[2]+3*x[3]+5*x[4])
}
newSuitnoCurrent <- cmpfun(newSuitnoCurrentU)
###stepSum calculates the difference between suitability in each time period####
stepSumU <- function(x){##x is dataframe with colums Period, Spp, CurrentSuit, NewSuit
    if(x[1] == 2025){
...
stepSum <- cmpfun(stepSumU)

###Calculate percent of models improving, same, or declining####
modDir2U <- function(x, direction){#columns 1,2,3,X,Currentsuit
    curr <- x[5]
...
modDir2 <- cmpfun(modDir2U)
###Flag model agreement in mid rotation period
modAgreeU <- function(x){
    m <- max(x)
    if(m >= 80){
        return("High")
    }else if(m >= 60){
...
modAgree <- cmpfun(modAgreeU)

combineListU <- function(...) {##Combine multiple dataframe in foreach loop
    mapply(FUN = rbind, ..., SIMPLIFY=FALSE)
}
combineList <- cmpfun(combineListU)

replaceZoneU <- function(x){##x is dataframe with MergedBGC, Table, SS_NoSpace
    pattern <- x[1]
    replacement <- x[2]
    SS_NoSpace <- x[3]
...
replaceZone <- cmpfun(replaceZoneU)

## Read in the latest R model
# fmodel = "./data/BECv10_AB_USAZones2018_RFmodel.Rdata"
fmodel = "./data/BGCv10_2000Pt_Rnd_Normal_1961_1990MSY_RFmodelKiriFinal"
if(verbose){
    write("Loading model...", stdout())
}
load(fmodel)
if(verbose){
    write("Loaded", stdout())
...
library(rzmq)
if(verbose){
    write("getting ZMQ context", stdout())
}
context <- init.context()
if(verbose){
    write("getting ZMQ socket", stdout())
}
socket <- init.socket(context, "ZMQ_REP")
if(verbose){
    write(paste("connecting zmq socket to port: ", port), stdout())
}
connect.socket(socket, paste("tcp://localhost:", port, sep=""))
if(verbose){
    write("connected.", stdout())
}

if(verbose){
    write("Loading Edatopic_v10.7.csv", stdout())
}
edatopename="./data/Edatopic_v10.7.csv"
E1 <-read.csv(edatopename,stringsAsFactors=FALSE,na.strings=".")
E1 <- E1[,-c(5)]


# rs = dbSendQuery(con, "select source, mergedbgc, ss_nospace, edatopic, codes from trees.edatopic_with_special_current")
# E1 = fetch(rs, n=-1)
## rename to match the camel case
# names(E1) = c('Source', 'MergedBGC','SS_NoSpace', 'Edatopic', 'Codes')

E1 <- unique(E1)

#################Import and Build Tree species suitability##############
if(verbose){
    write("Loading TreeSppSuit_v10.7.csv", stdout())
}
treesuit="./data/TreeSppSuit_v10.7.csv"
S1 <- read.csv(treesuit,stringsAsFactors=F,na.strings=".")
S1 <- unique(S1)
S1$Spp <- gsub("Sxw|Se|Sw","Sx",S1$Spp)

if(verbose){
    write("Loading crosswalk csv", stdout())
}
Crosswalk <- read.csv("./data/Crosswalk.csv")

###Import Data for stocking Standards####
if(verbose){
    write("Loading StockStands_v11 csv", stdout())
}
StandDat <- read.csv("./data/StockStands_v11.csv")
...
if(verbose){
    write("Loading StockingInfo.csv", stdout())
}
StandInfo <- read.csv("./data/StockingInfo.csv")
StandInfo <- unique(StandInfo)

if(verbose){
    write("Loading StockingHeight.csv", stdout())
}
StandHeight <- read.csv("./data/StockingHeight.csv", stringsAsFactors = FALSE)
StandHeight$SiteSeries <- gsub("[[:space:]]","",StandHeight$SiteSeries)

###OPTIONAL: Set up to run loops in parallel###
if(verbose){
    write("Setting up parallel processing cluster", stdout())
}
require(doParallel)
set.seed(123321)
coreNo <- makeCluster(detectCores() - 1, type='PSOCK', outfile="")
registerDoParallel(coreNo, cores = detectCores() - 1)
Cores <- as.numeric(detectCores()-1)
clusterEvalQ(coreNo, .libPaths("./lib"))

if(verbose){
    write("Entering initial loop", stdout())
}
while(1)
{
    msg <- receive.socket(socket, unserialize=FALSE);
    if(verbose){
        write("Recieved message on the pipeline", stdout())
    }
    ...
    if(verbose){
        write("Convert to object...", stdout())
    }
    # Y1 = read.csv(paste('./workdirs/', workdir, '/input.csv', sep=''), stringsAsFactors=F,na.strings=".")
    request = fromJSON(rawToChar(msg))
    Y1 = request$data
    if(verbose){
        write("Loaded", stdout())
    }
    start.time <- Sys.time()
    # Y1 <- fread(workdir, select = Columns, stringsAsFactors = FALSE, data.table = FALSE) #fread is faster than read.csv
    Y1 = Y1[,Columns]
    Y1 <- Y1[!is.na(Y1$ID1),]
    ...
    subset1 <- subset(Y1, select=c("Model", "SiteNo", "Latitude", "Longitude", "Elevation"))
    subset2 <- subset(Y1.sub, select=c("BGC", "BGC.pred"))

    Y2.sub <- cbind.data.frame(subset1, subset2)
    ...
    average <- request$params$average ##"No"##"Yes" ##

    #####Average points (if specified) and remove BGCs with low predictions####
    if(verbose){
        write("Finished initial data massage...", stdout())
    }
    ...
    #========================================================================================
    if(verbose){
        write("Parse through all points for this query", stdout())
    }

    # SiteNo.list is a list of all "siteNos", or IDs per point.
    # SiteNo.suit <-  foreach(SNL = SiteNo.list, .combine = rbind, .packages = c("doBy","foreach", "jsonlite")) %dopar% {
    SiteNo.suit <-  foreach(SNL = SiteNo.list, .combine = rbind) %do% {
        options(warn=0)
        # FuturePeriod.list is a list of the years - 2025, 255, 2085
        if(verbose){
            write("dopar", stdout())
        }

        SiteFuture.suit <- foreach(i = FuturePeriod.list, .combine = rbind, .packages = c("doBy","foreach"))  %dopar% {
            if(verbose){
                write("inner do", stdout())
            }
            ...
            # writeLines(toJSON(Y3.SSlist))
            # Y3.SSlist is a list of the site series for this specific point.
    ...
    if(verbose){
        write("FeasTrajLookup data.frame", stdout())
    }
    ...
    foreach(SS = unique(SiteNo.suit$SSCurrent[SiteNo.suit$SiteNo == Site]), .combine =  combineList, .multicombine = TRUE, .packages = "reshape") %dopar%{
        ...
        list(outputRaw,outputSum, StockStands) #raw, summary, referenceGuide
    }###end of loop

    # write.csv(allOutput[[2]], paste("./out_SummaryExample_", avg_boolean, ".csv", sep=""), row.names=F)
    # write.csv(allOutput[[1]], paste("./out_RawDataExample_", avg_boolean, ".csv", sep=""), row.names=F)
    # write.csv(allOutput[[3]], paste("./out_ReferenceGuide_", avg_boolean, ".csv", sep=""), row.names=F)
    # write.csv(BGC.pred.out,   paste("./out_BGC_pred_",       avg_boolean, ".csv", sep=""), row.names=F)
    # write.csv(allOutput[[2]], paste('./workdirs/', workdir, "/out_SummaryExample.csv", sep=''),  row.names=F)
    # write.csv(allOutput[[1]], paste('./workdirs/', workdir, "/out_RawDataExample.csv", sep=''),  row.names=F)
    # write.csv(allOutput[[3]], paste('./workdirs/', workdir, "/out_ReferenceGuide.csv", sep=''),  row.names=F)
    # write.csv(BGC.pred.out,   paste('./workdirs/', workdir, "/out_BGC_pred.csv", sep=''),        row.names=F)
    end.time <- Sys.time()
    results <- list(summary = allOutput[[2]], rawdata = allOutput[[1]], reference=allOutput[[3]], predictions = BGC.pred.out, timetaken=as.numeric(end.time-start.time))
    if(verbose){
        write("write to zmq stream", stdout())
    }
    send.raw.string(socket, toJSON(results));
    rm(results, allOutput)
    gc()
    # wait(); # get rid of zombie processes?
    if(verbose){
        write("Done!", stdout())
    }
}
